...
const DEFAULTS = {
  ...
  code_db_read:          "const { rows: [{resultVar}] } = await pool.query('SELECT {cols} FROM {table} WHERE {key} = $1', [{value}])  // if (!{resultVar}) { /* not found */ }",
  code_db_write:         "await pool.query('INSERT INTO {table} ({cols}) VALUES ({$params})', [{values}])  // or UPDATE: await pool.query('UPDATE {table} SET {col} = $1 WHERE {key} = $2', [val, key])",
  code_error_convention: "output.{rtnStsField} = {code}; return output",
